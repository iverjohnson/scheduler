...
}d[_]=(d[_-1]||0)+l.rows[_].cells[0].offsetHeight}d.unshift(0),l.parentNode.offsetHeight<l.parentNode.scrollHeight&&!l._h_fix}else if(s.length||"visible"!=this._els.dhx_multi_day[0].style.visibility||(d[0]=-1),s.length||-1==d[0]){var g=(l.parentNode.childNodes,(d[0]+1)*n+1+"px");o.style.top=this._els.dhx_cal_navline[0].offsetHeight+this._els.dhx_cal_header[0].offsetHeight+parseInt(g,10)+"px",o.style.height=this._obj.offsetHeight-parseInt(o.style.top,10)-(this.xy.margin_top||0)+"px";var f=this._els.dhx_multi_day[0];

f.style.height=g,f.style.visibility=-1==d[0]?"hidden":"visible",f=this._els.dhx_multi_day[1],f.style.height=g,f.style.visibility=-1==d[0]?"hidden":"visible",f.className=d[0]?"dhx_multi_day_icon":"dhx_multi_day_icon_small",this._dy_shift=(d[0]+1)*n,d[0]=0}}return s};var s=["dhx_cal_navline","dhx_cal_header","dhx_multi_day","dhx_cal_data"],a=function(t){for(var i=0,a=0;a<s.length;a++){var n=s[a],r=e._els[n]?e._els[n][0]:null,d=0;switch(n){case"dhx_cal_navline":case"dhx_cal_header":d=parseInt(r.style.height,10);
