...
dataProcessor.wrap("afterUpdate",function(that,xml,id){
	...
	var atag=dhx4.ajax.xpath("//data/action", xml.xmlDoc);
	if (!atag){
		that._log("&nbsp;No actions found");
		var atag=dhx4.ajax.xmltop("data", xml.xmlDoc);
		if (!atag || atag.tagName == "DIV") that._log("&nbsp;XML not valid");
